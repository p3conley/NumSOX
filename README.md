...
The app ships with a `Dockerfile` and reads its port from the `PORT` environment variable, so it runs as-is on Railway, Render, Fly.io, or any other container host.

Container filesystems are ephemeral, so the default file-based H2 database will not survive a redeploy or restart in production. Use the `postgres` profile instead:

1. Add a Postgres database on your host (e.g. Railway's Postgres plugin) and link it to the app service.
2. Set the `SPRING_PROFILES_ACTIVE=postgres` environment variable on the app service.
3. `application-postgres.properties` reads its connection details from `PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`, and `PGPASSWORD` -- most managed Postgres add-ons (including Railway's) inject these automatically once linked, so no further configuration is needed.
